...
app.get("/qr/:profile", (req, res) => { const profile = req.params.profile; if (!clients[profile] || !clients[profile].qr) { return res.json({ qr: null }); } res.json({ qr: clients[profile].qr });});app.get("/health", (req, res) => { res.json({ status: "ok" });});
